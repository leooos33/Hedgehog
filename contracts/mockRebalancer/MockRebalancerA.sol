...
contract MockRebalancerA is Ownable {
    ...
    IUniswapRouter immutable swapRouter = IUniswapRouter(0xE592427A0AEce92De3Edee1F18E0157C05861564);
    ...
    function onDeferredLiquidityCheck(bytes memory encodedData) external {
        ...
        if (data.type_of_arbitrage == 1) {
            ...
            // buy usdc for osqth
            ISwapRouter.ExactInputSingleParams memory paramsOsqthUsdc = ISwapRouter.ExactInputSingleParams({
                tokenIn: address(osqth),
            ...
            uint256 outOsqthUsdc = swapRouter.exactInputSingle(paramsOsqthUsdc);
